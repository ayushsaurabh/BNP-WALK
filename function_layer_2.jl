function collapse_trajectory!(position_x, 
				position_y, 
				change_points_time, 
				change_points_index,
				loads,
	...
	if n_change_points > 2
		for change_point in 2:max_n_change_points-1 
			if loads[change_point] != 0
				active_change_point_index = 
					findall(x-> x == change_point, view(loads_active, 1:n_change_points))[1]
					jump_size = sqrt((position_x[loads_active[active_change_point_index]] - 
							  position_x[loads_active[active_change_point_index-1]])^2 
							 +(position_y[loads_active[active_change_point_index]] - 
							   position_y[loads_active[active_change_point_index-1]])^2)
				if jump_size < minimum_allowed_jump_size 
					#println("collapse_occurred")
					#@show change_point
					#@show jump_size/(1.0e-9)
					loads[change_point] = 0
					position_x[loads_active[active_change_point_index-1]] = 
						(position_x[loads_active[active_change_point_index]] + 
						 position_x[loads_active[active_change_point_index-1]])/2.0
					position_y[loads_active[active_change_point_index-1]] = 
						(position_y[loads_active[active_change_point_index]] +
						 position_y[loads_active[active_change_point_index-1]])/2.0
...
function get_likelihood_diffusion_coeff(draw, 
		position_x, 
		position_y, 
		change_points_time, 
		change_points_index,
	...
	return old_log_likelihood, proposed_log_likelihood
end

function get_likelihood_dwell_gamma(draw, 
		change_points_time, 
		change_points_index,
		dwell_gamma_shape,
		dwell_gamma_mean,
